...
// ===----------------------------------------------------------------------===
    ...
    @Suite
    struct RealDatabaseFilterTests: DiskUsingTest {
        var additionalPath: String { "/Filter" }
        ...
        func testDoubleFieldGreaterThanOrEqualToDoubleField() async throws {
            ...
            #expect(results
                .count == 3) // Mia (500 >= -50), Charlie (0 >= 0), matching (-10 >= -100)
        }
        ...
        func testSortRule() async throws {
            ...
            let descriptor = try FetchDescriptor(
                filter: #Filter<V0_0_1.User> { _ in true },
