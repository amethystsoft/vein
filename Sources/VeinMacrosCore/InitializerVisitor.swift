    ...
    override func visit(_ node: SequenceExprSyntax) -> SyntaxVisitorContinueKind {
            ...
            baseDecl.baseName.text == "self"
        {
            ...
            if forbiddenSelfAssignments.contains(assignedPropertyName) {
                context.diagnose(.init(node: Syntax(memberAccess), message: ErrorDiag(message: "Illegal assignment to relationship property in initializer. Relationships require the model to be managed by a context.")))
            }
            return .visitChildren
        }
        ...
        if let declRef = lhs.as(DeclReferenceExprSyntax.self) {
            ...
            if forbiddenSelfAssignments.contains(assignedVariableName) {
                let allLocalVariables = localVariableScopes.flatMap { $0 }
                
                if !allLocalVariables.contains(assignedVariableName) {
                    context.diagnose(.init(node: Syntax(declRef), message: ErrorDiag(message: "Illegal assignment to relationship property in initializer. Relationships require the model to be managed by a context.")))
                }
            }
        }
